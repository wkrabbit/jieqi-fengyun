...
### v0.8.5
- 联机胜利窗口残留修复：backToLobby/newGame 完全重置游戏状态
- 恢复暗子吃子信息不对称设计（吃方可见，被吃方不可见）

### v0.8.4
- 玩家头像和用户名：己方在左下角，对方在右上角（视角自适应）
- 新增 TimerBox 组件：计时框跟随头像位置，己方右侧对方左侧
- 棋盘翻转修复为完整 180° 旋转（row=9-row, col=8-col）
- 得子区域改为上下布局：己方得子在棋盘下方，对方得子在棋盘上方
- SidePanel 联机模式隐藏旧 TimerDisplay

### v0.8.3
- 联机模式棋盘翻转：黑方视角黑棋在下，红方在上
- coordinates.ts 增加 flipped 参数，视觉行自动变换为 9-row
- 楚河汉界文字翻转时反转显示
- 得子区域随视角翻转，己方得子始终在左侧
- 点击坐标、棋子高亮、选中光晕全部适配翻转

### v0.8.2
- 快速匹配改为房间列表：展示可用房间，点击房间号加入，支持刷新
- 移除 matchQueue 队列机制，改为 Map 按需查询
- SidePanel 加宽 w-44→w-64（176→256px），聊天区域更大

### v0.8.1
- 联机胜利弹窗残留修复：返回大厅时重置 game.phase
- 快速匹配队列清理：断线/创建/加入房间时无条件从匹配队列移除
- 聊天框加大：SidePanel 宽度 176→256px，消息区高度 220→320px，字号 xs→sm
- 移除 gameStore.ts 中未使用的 getPositionType 导入

### v0.8.0
- 作弊暗棋走法修复：作弊仅改变翻出类型，翻出前保持暗棋（位置）走法规则

### v0.7.9
- 暗子延迟随机生成 (deferredIdentity)：暗棋初始类型翻棋时从剩余池动态抽取
- 作弊按剩余池动态调整限额
- 新增 deferredIdentity.test.ts
